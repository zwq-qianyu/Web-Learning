...
class DecisionTreeClassifier():
    '''决策树分类器'''
    def __init__(self, max_depth=2, min_samples_leaf=1):      # max_depth --- 最大深度
        '''初始化'''
        self.tree_ = None
    ...
    def create_tree(self, X, y, curren_depth=1):
        '''创建决策树'''
        if curren_depth > self.max_depth:
            return None
        ...
        node.children_left = self.create_tree(X_left, y_left, curren_depth + 1)
        if node.children_left is None:
            label = Counter(y_left).most_common(1)[0][0]
            node.children_left = Node(l=label)

        node.children_right = self.create_tree(X_right, y_right, curren_depth + 1)
